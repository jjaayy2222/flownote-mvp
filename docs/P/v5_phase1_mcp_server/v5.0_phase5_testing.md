...
1. **File Creation and Classification**
   - ✅ Obsidian 파일 생성 → FlowNote 분류 → PARA 폴더 이동
   - ✅ `move_file_to_para` 동작 검증

2. **Conflict Detection**
   - ✅ 3-way 충돌 감지 (local vs remote vs last_synced)
   - ✅ `detect_conflict_3way` 로직 검증
...
3. **Test MCP Tools:**
   - Ask Claude: "내 노트에서 '프로젝트' 관련 내용 찾아줘"
   - Verify that Claude uses the `search_notes` tool

4. **Test Classification:**
   - Ask Claude: "이 텍스트를 분류해줘: 오늘 프로젝트 회의가 있습니다."
