...
from backend.models.conflict import ConflictRecord, ConflictReport
...
save_func = (
    save_json_log.__func__ if hasattr(save_json_log, "__func__") else save_json_log
)
...
async def classify_text(request: ClassifyRequest):
    ...
    try:
        ...
        logger.info(
            f"   - Context Keywords: {list(user_context['context_keywords'].keys())}"
        )
        ...
        logger.info(
            f"🔍 키워드 분류 시작 (Instance ID: {keyword_classifier.instance_id})"
        )
        ...
        if not isinstance(new_keyword_tags, list):
            ...
            new_keyword_tags = [
                str(tag) for tag in new_keyword_tags if str(tag).strip()
            ]
        ...
        logger.info(
            f"   - User Context Matched: {keyword_result.get('user_context_matched')}"
        )
        ...
        logger.info(
            f"   - Keyword Tags: {conflict_result.get('keyword_tags', new_keyword_tags)}"
        )
        logger.info(
            f"   - Conflict Detected: {conflict_result.get('conflict_detected')}"
        )
        ...
        try:
            ...
            ctx_data[uid]["recent_categories"] = ctx_data[uid]["recent_categories"][
                -10:
            ]
        ...
        response = ClassifyResponse(
            ...
            user_areas=areas_list or [],
            user_context=user_context,
            context_injected=bool(areas_list),
            log_info=log_info,
            csv_log_result=csv_log_result,
        )

        logger.info(
            f"✅ 전체 분류 완료 → {response.category} | 키워드 {len(response.keyword_tags)}개"
        )
...
async def classify_file_main(
    ...
    try:
        ...
        if user_profile:
            ...
            areas_list = (
                [a.strip() for a in stored_areas.split(",")]
                if stored_areas
                else areas_list or []
            )
            stored_interests = user_profile.get("interests", "")
            interests_list = (
                [i.strip() for i in stored_interests.split(",")]
                if stored_interests
                else interests_list or []
            )
            logger.info(
                f"✅ 프로필 로드 성공: {occupation_value}, areas={len(areas_list)}개"
            )
        else:
        ...
        try:
            csv_log_result = data_manager.log_classification(
                user_id=effective_user_id,
                file_name=file_id or "text_input",
                ai_prediction=final_category,
                user_selected=None,
                confidence=conflict_result.get("confidence", 0.0),
            )
...
async def classify_file_advanced(file: UploadFile = File(...)):
    ...
    try:
        ...
        # 🔥 Sync 버전 호출!
        try:
            para_result = await run_para_agent(text=sample_text, metadata=metadata)
        ...
        raise HTTPException(
            status_code=400, detail="파일 인코딩 오류. UTF-8 파일만 지원합니다."
        )
...
@router.post(
    ...
    tags=["Classification", "Storage", "Save"],
)
...
@router.get(
    "/metadata/{file_id}",
    response_model=Dict,
    tags=["Classification", "Metadata", "Query"],
)
    ...
    try:
        ...
        if request.user_id and user_areas:
            result = injector.inject_context_from_user_id(
                user_id=request.user_id, ai_result=result
            )
...
@router.post(
    "/metadata",
    response_model=ClassifyResponse,
    tags=["Classification", "Advanced", "Metadata Based"],
)
...
@router.post(
    "/hybrid",
    response_model=ClassifyResponse,
    tags=["Classification", "Advanced", "Hybrid"],
)
