...
class ConfidenceCalculator:
    ...
    def calculate(
        self, scores: Dict[str, float], features: Optional[Dict[str, Any]] = None
    ) -> ConfidenceResult:
        """
        ...
        adjusted_score = self._apply_adjustments(
            base_score, scores, features, reasons, adjustments
        )
    ...
    def _apply_adjustments(
        ...
        if self._check_agreement(scores):
            add_adjustment(
                "boost", "Agreement Boost", self._ADJUSTMENT_FACTORS["agreement_boost"]
            )
        elif self._check_disagreement(scores):
            add_adjustment(
                "penalty",
                "Disagreement Penalty",
                self._ADJUSTMENT_FACTORS["disagreement_penalty"],
            )
        ...
        if access_freq >= 0.5:
            add_adjustment(
                ...
                self._ADJUSTMENT_FACTORS["high_freq_boost"],
            )
        ...
        if days_since_edit <= 7:
            add_adjustment(
                "boost",
                "Recent Edit Boost",
                self._ADJUSTMENT_FACTORS["recent_edit_boost"],
            )
        ...
        if "text_length" in features:
            ...
            if text_length < 50:
                add_adjustment(
                    "penalty",
                    "Low Info Penalty",
                    self._ADJUSTMENT_FACTORS["low_info_penalty"],
                )
