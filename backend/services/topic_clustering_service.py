...
class ClusterCacheConfig:
    """
    [리뷰반영] 캐시 버전 관리를 캡슐화(Encapsulation)한 매니저 클래스.
    환경 변수 로딩, 지연 평가(lru_cache), 핫 리로드(clear_cache)를 응집도 있게 관리한다.
    """
    ENV_KEY = "CLUSTER_CACHE_VERSION"
    DEFAULT_VERSION = "v1"

    @staticmethod
    @lru_cache(maxsize=1)
    def get_version() -> str:
        raw_val = os.environ.get(ClusterCacheConfig.ENV_KEY)
        
        if raw_val is None:
            logger.info(
                "[TOPIC_CLUSTERING] %s 환경 변수가 미설정되었습니다. 기본값 %r을 사용합니다.",
                ClusterCacheConfig.ENV_KEY,
                ClusterCacheConfig.DEFAULT_VERSION,
            )
            return ClusterCacheConfig.DEFAULT_VERSION
    ...
    def clear_cache() -> None:
        ...
        """
        ClusterCacheConfig.get_version.cache_clear()
...
def _build_cluster_cache_key(hashed_user_id: str) -> str:
    ...
    version = ClusterCacheConfig.get_version()
