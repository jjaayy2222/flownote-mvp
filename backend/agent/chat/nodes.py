...
# - 의도적 제외: 16자리 이상의 연속된 숫자는 일반적인 전화번호 범위를 벗어나므로 무시할 수 있음.
# - 탐지 대상: 한국형(01x-xxx-xxxx) 및 +국가번호가 포함된 국제 규격(E.164 유사 포맷).
# - 오탐 방지: (?<!\d)와 (?!\d) 룩어라운드를 사용하여 타임스탬프 일부가 잘려 매칭되는 것을 차단.
# - 구분자: 하이픈(-), 공백( ), 점(.) 및 구분자가 없는 밀집 포맷 지원.
_PHONE_PATTERN = re.compile(
    r"(?<!\d)(?:\+?\d{1,3}[- .]?)?\(?0?\d{1,4}\)?[- .]?\d{3,5}[- .]?\d{4}(?!\d)"
)
