...
8. [RAG 품질 평가 및 성능 (v8.0)](#8-rag-품질-평가-및-성능-v80)
9. [자가 적응형 지능 (v9.0)](#9-자가-적응형-지능-v90)
...
## 8. RAG 품질 평가 및 성능 (v8.0)

v8.0에서는 RAG 시스템의 신뢰성을 측정하고 성능을 극대화하기 위한 도구들이 추가되었습니다.

### 8.1 Golden Dataset 추출
사용자의 긍정/부정 피드백이 쌓이면, 시스템은 이를 바탕으로 '정답 셋(Golden Dataset)'을 자동으로 생성합니다.
- **수집**: 대화 로그 분석 및 피드백 태그 필터링
- **활용**: 새로운 검색 알고리즘이나 프롬프트 변경 시 성능 비교 기준(Baseline)으로 사용

### 8.2 정밀 평가 프레임워크
`tests/e2e/test_rag_search_quality.py`를 통해 검색 정확도를 측정할 수 있습니다.
- **Precision**: 검색 결과 중 실제 관련 있는 문서의 비율
- **Recall**: 전체 관련 문서 중 검색된 문서의 비율
- **Hallucination Check**: AI 답변이 실제 문서 근거에 기반했는지 검증

### 8.3 성능 최적화 (Backend)
- **LLM Caching**: 동일한 설정의 LLM 클라이언트를 재사용하여 세션 시작 지연 시간 단축
- **Redis Pipelining**: 다수의 Redis I/O 작업을 묶어 처리함으로써 네트워크 오버헤드 최소화

---

## 9. 자가 적응형 지능 (v9.0)

v9.0 **Adaptive Intelligence**는 시스템이 사용자의 데이터에 맞춰 스스로 진화하는 엔진을 도입했습니다.

### 9.1 자율 파인튜닝 (Adaptive Fine-tuning)
사용자의 특수한 문서 도메인(예: 의학, 법률, 특정 기업 내부 용어)에 맞춰 AI 분류 모델을 스스로 학습시킵니다.
- **작동**: 학습에 적합한 데이터가 임계치 이상 쌓이면 OpenAI Fine-tuning Job을 자동으로 생성합니다.
- **모니터링**: `FineTuningService`가 백그라운드에서 학습 상태를 폴링하며 완료 시점에 Redis 상태를 갱신합니다.

### 9.2 운영 관측성 및 안전 장치
- **시스템 태그**: 로그에 `event` 및 `meta_intentional_warning` 필드가 추가되어, 운영자가 모니터링 대시보드에서 시스템의 자율 행동을 필터링하여 감시할 수 있습니다.
- **데이터 무결성**: 핵심 상태 값(status 등)은 외부 메타데이터로 오버라이드할 수 없도록 보호되어 시스템의 안정성을 유지합니다.
