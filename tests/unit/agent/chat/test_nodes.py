...
class TestChatNodes(unittest.TestCase):
    def _make_history(self, rating: str, count: int) -> list[dict]:
        return [{"rating": rating} for _ in range(count)]
    ...
    def test_should_fallback_under_threshold(self):
        ...
        downs = self._make_history(RATING_DOWN, FALLBACK_THRESHOLD - 1)
    ...
    def test_should_fallback_reach_threshold(self):
        ...
        downs = self._make_history(RATING_DOWN, FALLBACK_THRESHOLD)
        state = {"feedback_history": downs}
    ...
    def test_should_fallback_mixed_recent(self):
        ...
        history = self._make_history(RATING_UP, FALLBACK_WINDOW_SIZE - FALLBACK_THRESHOLD)
        history.extend(self._make_history(RATING_DOWN, FALLBACK_THRESHOLD))
    ...
    def test_should_fallback_old_downs(self):
        ...
        history = self._make_history(RATING_DOWN, FALLBACK_THRESHOLD)
        history.extend(self._make_history(RATING_UP, FALLBACK_WINDOW_SIZE))
        state = {"feedback_history": history}
        self.assertEqual(should_fallback(state), ROUTE_STANDARD_RAG)
