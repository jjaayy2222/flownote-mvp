...
class TestEndToEndWorkflow:
    ...
    def test_02_file_classification(self, setup):
        ...
        for clf in classification_results["classifications"]:
            print(f"✅ 분류 완료: {clf['filename']} → {clf['para_category']} (신뢰도: {clf['confidence']})")
    ...
    def test_05_dashboard_integration(self, setup):
        ...
        print(f"   - 분류된 파일: {dashboard_results['dashboard_data']['classified_files']}")
        print(f"   - 평균 신뢰도: {dashboard_results['dashboard_data']['statistics']['total_confidence_avg']}")
