...
async def test_gpt_failure_fallback(classification_service):
    ...
    with patch("backend.services.classification_service.run_para_agent", side_effect=Exception("GPT API Error")) as mock_para:
...
async def test_log_save_failure_resilience(classification_service):
    ...
    with patch.object(classification_service, '_save_results', side_effect=Exception("Disk Full")):
        with patch("backend.services.classification_service.run_para_agent", new_callable=AsyncMock) as mock_para:
            mock_para.return_value = {"category": "Projects", "confidence": 0.9}
